...
class TanishqOriginalProxyCachingHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        # 1. Map root requests to the copied original index.html
        if self.path == '/' or self.path == '/index.html':
            print("[Server] Serving original Tanishq index.html.")
...
if __name__ == '__main__':
    # Serve Tanishq on port 8081
    server_address = ('', 8081)
    httpd = HTTPServer(server_address, TanishqOriginalProxyCachingHandler)
    print("Tanishq Cloned Original Server running on http://localhost:8081/ ...")
    httpd.serve_forever()
